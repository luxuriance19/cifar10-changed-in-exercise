...
def distorted_inputs():
  ...
  """ add to see the image after distorted"""
  tf.summary.image("distored_image", images)
...
def loss(logits, labels):
  ...
  cross_entropy_mean = tf.reduce_mean(-tf.reduce_sum(y_true*tf.log(logits),axis=1))
...
def train(total_loss, global_step):
  ...
  with tf.control_dependencies([loss_averages_op]):
    ...
    grads = opt.compute_gradients(total_loss) # first part of minimize, return a list of (gradient,variable) pairs

  # Apply gradients.
  apply_gradient_op = opt.apply_gradients(grads, global_step=global_step) #second part of minimize

  # Add histograms for trainable variables.
  for var in tf.trainable_variables(): # all variables created with trainable=True
